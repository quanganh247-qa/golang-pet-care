...
func (controller *ServiceController) DeleteService(ctx *gin.Context) {
	...
	err := controller.service.deleteServiceService(ctx, req.ServiceID)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, util.ErrorResponse(err))
		return
	}
...
func (controller *ServiceController) GetAllServices(ctx *gin.Context) {
	services, err := controller.service.getAllServicesService(ctx)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, util.ErrorResponse(err))
		return
	}
	ctx.JSON(http.StatusOK, util.SuccessResponse("Success", services))
}
